/** NANP US area code -> USPS state code. Mirrors the smartr8 Pages copy
 *  (functions/_lib/areaCodeState.ts) and the standalone backfill script. Used as a
 *  FALLBACK to resolve a recipient timezone when leads.property_state is blank
 *  (the funnels collect phone but not state). Best-effort: area code approximates
 *  the state. Non-geographic codes (toll-free 8xx, 900, etc.) -> "". */
